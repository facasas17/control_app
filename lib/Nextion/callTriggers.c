 ...
 * Code - Public
 ******************************************************************************/
void EasyNex_callTriggerFunction(void)
{
    char _tempRead;  
    
    // We read the next byte, which, according to our protocol, is the < Trigger ID >
    // From Nextion we send: < printh 23 02 54 xx >
    // (where xx is the trigger id in HEX, 01 for 1, 02 for 2, ... 0A for 10 etc)
    UART_ReadData(myNextion.uart_port, &_tempRead, 1);
